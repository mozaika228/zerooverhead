...
static void zh_slab_list_push(zh_slab_class_t* cls, zh_slab_t* slab) {
  slab->next = cls->partial;
  slab->prev = 0;
  if (cls->partial) cls->partial->prev = slab;
  cls->partial = slab;
  slab->in_list = 1;
}
...
void* zh_alloc_small(size_t size) {
  ...
  if (!slab || !slab->free_list) {
    ...
    slab = cls->partial;
    if (slab) zh_slab_list_remove(cls, slab);
  ...
  if (slab->free_count == 0 && slab->in_list) {
    zh_slab_class_t* cls = &g_classes[idx];
    zh_spinlock_lock(&cls->lock);
...
void zh_free_small(void* ptr) {
  ...
  *(void**)h = slab->free_list;
  ...
  if (prev_free == 0) {
    ...
    zh_spinlock_lock(&cls->lock);
    if (!slab->in_list) zh_slab_list_push(cls, slab);
    zh_spinlock_unlock(&cls->lock);
  }
}
...
void zh_free_large(void* ptr) {
  ...
  if (h->magic != ZH_LARGE_MAGIC) return;
...
size_t zh_usable_large(void* ptr) {
  if (!ptr) return 0;
  zh_large_header_t* h = ((zh_large_header_t*)ptr) - 1;
  if (h->magic != ZH_LARGE_MAGIC) return 0;
  return h->size;
}
